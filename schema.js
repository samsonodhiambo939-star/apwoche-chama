...
async function initDatabase() {
  if (isPg) {
    await db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS contributions (id SERIAL PRIMARY KEY, member_id INTEGER NOT NULL REFERENCES members(id) ON DELETE CASCADE, fund_type_id INTEGER NOT NULL REFERENCES fund_types(id), cycle_id INTEGER NOT NULL REFERENCES cycles(id), amount REAL NOT NULL DEFAULT 0, status TEXT DEFAULT 'pending', created_by INTEGER, created_by_role TEXT, created_at TIMESTAMP DEFAULT NOW(), UNIQUE(member_id, fund_type_id, cycle_id));
      ...
      CREATE TABLE IF NOT EXISTS loans (id SERIAL PRIMARY KEY, member_id INTEGER NOT NULL REFERENCES members(id) ON DELETE CASCADE, amount REAL NOT NULL, interest_rate REAL DEFAULT 10, amount_due REAL NOT NULL, issued_date TEXT, due_date TEXT, approved_date TEXT, status TEXT DEFAULT 'pending', paid_amount REAL DEFAULT 0, defaulted_penalty REAL DEFAULT 0, created_by INTEGER, created_by_role TEXT, created_at TIMESTAMP DEFAULT NOW());
      CREATE TABLE IF NOT EXISTS fines (id SERIAL PRIMARY KEY, member_id INTEGER NOT NULL REFERENCES members(id) ON DELETE CASCADE, amount REAL NOT NULL, balance REAL NOT NULL, reason TEXT, status TEXT DEFAULT 'pending', created_by INTEGER, created_by_role TEXT, created_at TIMESTAMP DEFAULT NOW());
    ...
    db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS contributions (id INTEGER PRIMARY KEY AUTOINCREMENT, member_id INTEGER NOT NULL, fund_type_id INTEGER NOT NULL, cycle_id INTEGER NOT NULL, amount REAL NOT NULL DEFAULT 0, status TEXT DEFAULT 'pending', created_by INTEGER, created_by_role TEXT, created_at TEXT DEFAULT (datetime('now')), UNIQUE(member_id, fund_type_id, cycle_id), FOREIGN KEY (member_id) REFERENCES members(id) ON DELETE CASCADE, FOREIGN KEY (fund_type_id) REFERENCES fund_types(id), FOREIGN KEY (cycle_id) REFERENCES cycles(id));
      ...
      CREATE TABLE IF NOT EXISTS loans (id INTEGER PRIMARY KEY AUTOINCREMENT, member_id INTEGER NOT NULL, amount REAL NOT NULL, interest_rate REAL DEFAULT 10, amount_due REAL NOT NULL, issued_date TEXT, due_date TEXT, approved_date TEXT, status TEXT DEFAULT 'pending', paid_amount REAL DEFAULT 0, defaulted_penalty REAL DEFAULT 0, created_by INTEGER, created_by_role TEXT, created_at TEXT DEFAULT (datetime('now')), FOREIGN KEY (member_id) REFERENCES members(id) ON DELETE CASCADE);
      CREATE TABLE IF NOT EXISTS fines (id INTEGER PRIMARY KEY AUTOINCREMENT, member_id INTEGER NOT NULL, amount REAL NOT NULL, balance REAL NOT NULL, reason TEXT, status TEXT DEFAULT 'pending', created_by INTEGER, created_by_role TEXT, created_at TEXT DEFAULT (datetime('now')), FOREIGN KEY (member_id) REFERENCES members(id) ON DELETE CASCADE);
  ...
  // Column migrations that run on BOTH engines (duplicate-column errors are ignored)
  const migrateCols = async (table, cols) => {
    for (const col of cols) {
      try {
        if (isPg) { await db.prepare(`ALTER TABLE ${table} ADD COLUMN ${col}`).run(); }
        else { db.prepare(`ALTER TABLE ${table} ADD COLUMN ${col}`).run(); }
      } catch(e) {}
    }
  };
  if (isPg) {
  ...
  await migrateCols('contributions', ['created_by INTEGER', "created_by_role TEXT"]);
  await migrateCols('loans', ['created_by INTEGER', "created_by_role TEXT"]);
  await migrateCols('fines', ['created_by INTEGER', "created_by_role TEXT"]);
  await migrateCols('payment_requests', ['created_by INTEGER', "created_by_role TEXT"]);
